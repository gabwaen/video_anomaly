...
if __name__ == '__main__':
	...
	filename = f"./RWF-2000/val/Fight/{filename}.avi"
	# filename = f"./RWF-2000/val/NonFight/{filename}.avi"
...
if __name__ == '__main__':
	...
	# 	for instant_ts in sorted(list(move_hist.keys())):
	# 		instant = move_hist[instant_ts]
	...
	# # 	hdr = ['ID', 'Radius'] + [f'Instant {i}' for i in sorted(list(move_hist.keys()))]
	# # 	outfile.write(sep.join(hdr))
	# # 	outfile.write('\n')
	# # 	outfile.write('\n'.join(txt_data))
	...
	# 		['ID', 'Radius']
	# 		+ 
	# 		[f'Instant {i}' for i in sorted(list(move_hist.keys()))]
	# 		+
	# 		['\n']
	# 	)
	# 	+
	...
	# table = pd.read_csv(tabledata, sep='\t')
